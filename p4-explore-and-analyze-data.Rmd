...
```{r echo=FALSE, message=FALSE, warning=FALSE, Daily_Pickups}
ggplot(uber.by_day,
       aes(x = day,
           y = rides)) +
...
Friday, then peaking on the weekend.  It's also interesting to note the 
differences in ranges for each day.  I expected Friday's variation to be larger 
than any other day but it was tighter and consistently larger instead.
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Location_Pickups}
...
because the data was so spread out.
...
the pickup data by location and time for some of the histograms and boxplots.  
I had a dilema when it came to the taxi lookup dataset because some of the 
location data was missing.  It was a small subset (about 6000 observations) but 
I did not want to throw away the data.  Instead I kept all observations for the
time period analyses and excluded them from the location analyses.
...
would be interesting to look at ridership data for other cities to look for 
correlations.

I used the dplyr and tidyr packages to reshape the data.  This helped parse 
the time data to get month, date, day and hour variables.  It also helped when I
...
and I feel a certain level of comfort when I see his name in a package's 
documentation.
...
instead, demand grows steadily from 6am through midnight.  The growth on 
Saturday extends through this entire time period while Sunday demand tapers off 
around 6pm as riders prepare for the coming work week.
...
but instead of demand plateauing around 7pm, it continued to rise through 11 pm.
This spike in demand was most likely due to the festivals taking place in the
area that weekend.  I was able to find a 
...
Uber drivers could benefit from some of this data.  As a former driver, I 
understand the struggle to create a feasible work schedule.  Because they are 
...
create the plot and grapped the background from 
...
was by far the most popular region, followed by remote areas in Brooklyn and
...
was massive (between 2 and 460732 pickups).  Finally, I played around with the 
...
notation to decimal notation to make it more human-readable.
...
6am, then steadily rose throughout the rest of the day.  It also consistently 
started higher at midnight because people usually go out more on Friday and 
...
to see how demand changed over time and how it varied by location.  It is 
important to know the market in order to turn a profit.  Being in the right 
...
spiked so much around 6pm and that there really wasn't much of a lunch rush 
Monday-Friday.
...
area they would be better equipped to maximizetheir time and profit.  Drivers 
